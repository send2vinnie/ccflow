...
namespace BP.CT.T1Return
{
    public class HeLiuNodeReturn : TestBase
    {
        ...
        public HeLiuNodeReturn()
        {
            ...
            this.DescIt = "以demo中的 FlowNo=006 月销售总结(同表单分合流), 为测试案例.";
        ...
        /// <summary>
        /// 说明 ：以demo中的 FlowNo=006 月销售总结(同表单分合流), 为测试案例。
        ...
        public override void Do()
        {
            ...
            //创建空白工作, 发起开始节点.
            workID = BP.WF.Dev2Interface.Node_CreateBlankWork(fk_flow, null, null, WebUser.No, null, 0);
            ...
            // 执行退回子线程1.
            BP.WF.Dev2Interface.Node_ReturnWork(fl.No, threadWorkID1, workID, 699, 602, "test msg1", false);
            ...
            gwl = new GenerWorkerList(threadWorkID1, 602, "zhoushengyu");
            if (gwl.IsPass == true)
                throw new Exception("@子线程不应该是已通过的状态.");
            #endregion 检查子线程数据是否正确？

            // 执行退回子线程2.
            BP.WF.Dev2Interface.Node_ReturnWork(fl.No, threadWorkID2, workID, 699, 602, "test msg2", false);
            ...
            gwl = new GenerWorkerList(threadWorkID1, 602, "zhangyifan");
            if (gwl.IsPass == true)
                throw new Exception("@子线程不应该是已通过的状态.");
            #endregion 检查子线程数据是否正确？
        }
        ...
        {
            //获取它的待办, 从而获取子线程id.
            var dt = Dev2Interface.DB_GenerEmpWorksOfDataTable((int)WFState.Runing, fk_flow);

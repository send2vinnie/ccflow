...
namespace BP.En
{
    ...
    public class Map
    {
        ...
        {
            return BP.Sys.CField.GetMyAttrs(en.GetNewEntities);
        }
        public Attrs GetChoseAttrs(Entities ens)
        {
            return BP.Sys.CField.GetMyAttrs(ens);
        }
        #endregion
            ...
            get
            {
                if (_isJs == -1)
                {
                    foreach (Attr attr in _attrs)
                    {
                        if (attr.AutoFullDoc == null)
                            continue;
                        if (attr.AutoFullWay == BP.Sys.AutoFullWay.Way1_JS)
                        {
                            _isJs = 1;
                            break;
                        }
            ...
            get
            {
                if (this._HisCfgAttrs == null)
                {
                    ...
                    if (Web.WebUser.No == "admin")
                    {
                        ...
                        this._HisCfgAttrs.AddBoolen("IsEnableDouclickGlo", true, this.ToE("IsEnableDouclickGlo","是否启动双击打开(应用全局)"));
                        ...
                        this._HisCfgAttrs.AddDDLSysEnum("MoveToShowWay", 0, this.ToE("MoveToShowWay","移动到显示方式"), true, false, "MoveToShowWay", "@0=不显示@1=下拉列表@2=平铺");

...
#warning 限期时间  town.HisNode.DeductDays-1
        ...
        public string AfterNodeSave()
        {
            ...
            catch (Exception ex)
            {
                ...
                try
                {
                    ...
                    msg += this.HisNode.HisNDEvents.DoEventNode(EventListOfNode.SendError, this.HisWork);
                    throw ex;
                }
                catch (Exception ex1)
                {
        ...
        private string AfterNodeSave_Do()
        {
            ...
            //this.rptGe =this.HisWorkFlow.hisr
                ...
                if (this.HisNode.IsStartNode)
                {
                    ...
                    rptGe.SetValByKey(GERptAttr.FlowStarter, WebUser.No + "," + WebUser.Name);
                ...
                if (reffunc.Count > 0)
                {
                    ...
                    foreach (BillTemplate func in reffunc)
                    {
                        ...
                        try
                        {
                            ...
                            BillInfo += "<img src='" + this.VirPath + "/Images/Btn/Word.gif' /><a href='" + BillUrl + "' target=_blank >" + func.Name + "</a>";
...
#warning 为了不让其显示在途的工作需要， =3 不是正常的处理模式。
        ...
        {
            //string tempFilePath = SystemConfig.PathOfTemp + "\\" + this.WorkID + "_" + this.HisNode.NodeID + ".doc";
            ...
            //    // 插入图片
            //    string FileName = SystemConfig.PathOfWorkDir + @"\VisualFlow\Data\FlowDesc\" + this.HisNode.FK_Flow + ".gif"; // 图片所在路径
            ...
            //catch (Exception ex)
            //{
            //    throw new Exception("生成word 文件期间出现错误可能是服务器没有正确安装office2007以上版本：错误信息：" + ex.Message);
            //}
            //// message = name + " 文档生成成功，以保存到C:CNSI下 ";
            //PubClass.OpenWordDoc(tempFilePath, this.HisNode.Name + "_" + this.HisWork.RecText + ".doc");
        }
        #endregion

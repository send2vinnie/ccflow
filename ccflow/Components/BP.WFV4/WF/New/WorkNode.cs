...
#warning 限期时间  town.HisNode.DeductDays-1
        ...
        /// </summary>
        /// <returns></returns>
        public Nodes GenerNextStepNodes()
        {
            return null;
        }
        /// <summary>
        ...
        /// </summary>
        /// <returns>转到的节点</returns>
        private string NodeSend_12(Node toND)
        {
            return NodeSend_11(toND);
        }
        /// <summary>
        /// 处理分流点向下发送
        /// </summary>
        /// <param name="toNDs">可能有多个节点</param>
        /// <returns></returns>
        private string NodeSend_24(Nodes toNDs)
        {
            return null;
        }
        /// <summary>
        ...
        /// </summary>
        /// <returns></returns>
        private string NodeSend_24(Node toND)
        {
            #region GenerFH
            GenerFH fh = new GenerFH();
            fh.FID = this.WorkID;
        ...
        public string AfterNodeSaveV2(Node jumpToNode, string jumpToEmp)
        {
            //设置跳转节点，如果有可以为null.
            ...
            // 第1.1: 检查是否可以处理当前的工作.
            if (BP.WF.Dev2Interface.Flow_CheckIsCanDoCurrentWork(this.WorkID, WebUser.No) == false)
                throw new Exception("@当前工作您已经处理完成.");
            ...
            try
            {
                ...
                switch (this.HisNode.HisRunModel)
                {
                    case RunModel.Ordinary: /* 1： 普通节点向下发送的*/
                        Node toND = this.GenerNextStepNode();
                    ...
                    case RunModel.FL: /* 2: 分流节点向下发送的*/
                        Nodes toNDs = this.GenerNextStepNodes();
                        if (toNDs.Count == 1)
                        {
                        ...
                        else
                        {
                            ...
                            foreach (Node nd in toNDs)
                            {
                                if (nd.HisRunModel != RunModel.SubThread)
                                    msgE += "@流程设计错误:分流(" + this.HisNode.Name + ")下面的节点不是子线程:(" + nd.Name + ")";
                            }
                            if (string.IsNullOrEmpty(msgE) == false)
                                throw new Exception(msgE);

                            /*异表单的分流点向下发送有多个子线程节点.*/
                            msg += this.NodeSend_24(toNDs);
                        }
                        break;
                    case RunModel.HL:  /* 3: 合流节点向下发送 */
                        Node toND3 = this.GenerNextStepNode();
        ...
        {
            return AfterNodeSaveV2(jumpToNode, jumpToEmp);

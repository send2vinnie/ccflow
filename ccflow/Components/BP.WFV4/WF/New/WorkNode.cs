...
#warning 限期时间  town.HisNode.DeductDays-1
        ...
        public Nodes Func_GenerNextStepNodes()
        {
            ...
            if (myNodes.Count == 0)
                throw new Exception(string.Format(this.ToE("WN10_1",
                    "@定义节点的方向条件错误:没有给从{0}节点到其它节点,定义转向条件."),
        ...
        /// </summary>
        /// <returns></returns>
        private string Func_CheckCompleteCondition()
        {
        ...
        /// <returns></returns>
        private string NodeSend_24_SameSheet(Node toND)
        {
            /*检查之前是否已经有了分合流程*/
            ...
            string spanNodes = this.SpanSubTheadNodes(toND);
            if (toND.FromNodes.Count != 1)
            {
                return StartNextWorkNodeHeLiu_WithFID_YiBu(toND);
            }

            GenerFH myfh = new GenerFH(this.HisWork.FID);
            if (myfh.FK_Node == toND.NodeID)
            {
                /* 说明不是第一次到这个节点上来了, 
                 * 比如：一条流程：
                 * A分流-> B普通-> C合流
            ...
            */
            ...
            myfh.Update(GenerFHAttr.FK_Node, toND.NodeID,
            ...
            // 复制报表上面的数据到合流点上去。
            DataTable dt = DBAccess.RunSQLReturnTable("SELECT * FROM ND" + int.Parse(toND.FK_Flow) + "Rpt WHERE OID=" + dbStr + "OID",
            ...
            if (athDBs.Count > 0)
            {
                ...
                foreach (FrmAttachmentDB athDB in athDBs)
                {
                    ...
                    athDB_N.Copy(athDB);
                    athDB_N.FK_MapData = "ND" + toND.NodeID;
                    athDB_N.MyPK = athDB_N.MyPK.Replace("ND" + this.HisNode.NodeID, "ND" + toND.NodeID) + "_" + idx;
                    athDB_N.FK_FrmAttachment = athDB_N.FK_FrmAttachment.Replace("ND" + this.HisNode.NodeID,
                       "ND" + toND.NodeID);
            ...
            if (eleDBs.Count > 0)
            {
                ...
                foreach (FrmEleDB eleDB in eleDBs)
                {
                    ...
                    eleDB_N.Copy(eleDB);
                    eleDB_N.FK_MapData = "ND" + toND.NodeID;
                    eleDB_N.MyPK = eleDB_N.MyPK.Replace("ND" + this.HisNode.NodeID, "ND" + toND.NodeID);
...
#warning 为了不让其显示在途的工作需要， =3 不是正常的处理模式。
            ...
            return "@当前工作已经完成，流程已经运行到合流节点[" + toND.Name + "]。@您的工作已经发送给如下人员[" + toEmpsStr + "]，<a href=\"javascript:WinOpen('" + this.VirPath + "/WF/Msg/SMS.aspx?WorkID=" + this.WorkID + "&FK_Node=" + toND.NodeID + "')\" >短信通知他们</a>。" + "@您是第一个到达此节点的同事.";
        }
        /// <summary>
        /// 合流点到普通点发送
        /// 1. 首先要检查完成率.
        ...
        /// </summary>
        /// <returns></returns>
        private string NodeSend_53_SameSheet(Node toNode)
        {
            return null;
        }
...
#warning edit by peng, 用来确定不同岗位集合的传递包含同一个人的处理方式。
        ...
        /// </summary>
        /// <returns></returns>
        public string FeiLiuStartUp_del()
        {
            #region GenerFH
            GenerFH fh = new GenerFH();
            fh.FID = this.WorkID;
         ...
         //   string msg = this.CheckCompleteCondition();
            ...
            switch (this.HisNode.HisNodeWorkType)
            {
                ...
                case NodeWorkType.WorkFHL:
                    ...
                    {
                        DataTable dt = DBAccess.RunSQLReturnTable("SELECT b.No,b.Name FROM WF_GenerWorkerList a,Port_Emp b WHERE a.FK_Emp=b.No AND IsPass=0 AND FID=" +dbStr+"FID",
                            "FID", this.HisWork.OID);
                        if (dt.Rows.Count != 0)
                        {
                            int i = BP.DA.DBAccess.RunSQL("UPDATE WF_GenerWorkerlist SET FID=0 WHERE FID=WorkID AND WorkID=" + dbStr+ "WorkID ",
                                "WorkID",this.WorkID);
                            if (i == 1)
                            {
                            ...
                            dt = DBAccess.RunSQLReturnTable("SELECT b.No,b.Name FROM WF_GenerWorkerList a,Port_Emp b WHERE a.FK_Emp=b.No AND a.IsPass=0 AND a.IsEnable=1 AND FID=" + dbStr+"FID",
                                "FID",this.HisWork.OID);
                            if (dt.Rows.Count != 0)
                            {
...
#warning 需要修改成标题生成规则。
        ...
        public string StartNextNode(Node nd)
        {
            ...
            if (this.HisNode.IsFL && nd.IsHL)
                throw new Exception("@流程设计错误，分合流节点不能连续出现，中间节点必须为子线程。");
                    ...
                    case NodeWorkType.WorkFHL: // 如果下一个节点是合流节点，或者是分合流节点。
                        ...
                        switch (this.HisNode.HisNodeWorkType)
                        {
                            case NodeWorkType.StartWork: /* 如果当前点是以上类型 */
                                throw new Exception("@流程设计错误:开始节点不能连接合流节点.");
                            case NodeWorkType.Work:
                                throw new Exception("@流程设计错误:请流程设计人员检查流程,以获取更多的信息.{与合流点连接的上一个节点必须是子线程节点，现在是普通工作节点，请在节点属性里修改节点类型}");
                            case NodeWorkType.SubThreadWork: /*子线程向合流发送的情况.*/
                                if (this.HisWork.FID == 0)
                                    msg = StartNextWorkNodeHeLiu_WithOutFID(nd);  /* 没有流程ID,比如: */
                                else
                                    msg = StartNextWorkNodeHeLiu_WithFID(nd);  /* 子线程向合流发送的情况 */
                                break;
                            default:

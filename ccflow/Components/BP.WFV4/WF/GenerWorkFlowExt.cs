...
namespace BP.WF
{
    ...
    public class GenerWorkFlowExt : Entity
    {
        ...
        public string Title
        {
            get
            {
                string s= this.GetValStringByKey(GenerWorkFlowExtAttr.Title);
                if (s == "")
                    return this.RDT;
                return s;
            }
        ...
        {
            // . clear bad worker .  
            DBAccess.RunSQLReturnTable("DELETE FROM WF_GenerWorkerlist WHERE WorkID IN ( select WorkID from WF_GenerWorkerlist WHERE WorkID not in (select WorkID from WF_GenerWorkFlowExt) )");

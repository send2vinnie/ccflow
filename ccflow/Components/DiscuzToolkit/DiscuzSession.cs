...
namespace Discuz.Toolkit
{
    public class DiscuzSession
    {
        ...
        {
            User[] users = this.GetUserInfoByEmail(email, User.FIELDS);
        ...
        /// </summary>
        /// <param name="email">email地址</param>
        /// <param name="fields">需要获取的用户属性</param>
        /// <returns></returns>
        public User[] GetUserInfoByEmail(string email, string[] fields)
        {
            List<DiscuzParam> param_list = new List<DiscuzParam>();
            if (session_info != null && !string.IsNullOrEmpty(session_info.SessionKey))
            {
            ...
            UserInfoResponse rsp = util.GetResponse<UserInfoResponse>("users.getInfoByEmail", param_list.ToArray());
            return rsp.Users;
        }
        ...
        public void LoginForClient(string userName, string passwd)
        {
            string postData = "username=" + userName + "&password=" + passwd;
            Util.GetResponseBytes(forum_url + "login.aspx", postData);
        }
    }
}

...
namespace BP.DA
{
    ...
    public class DBAccess
    {
       ...
       // private static bool lock_SQL_RunSQL = false;
        ...
        {
            ConnOfOLE connofora = (ConnOfOLE)DBAccess.GetAppCenterDBConn;
            OleDbConnection conn = connofora.Conn;
            try
            {
                if (conn == null)
                ...
                //   cmd.Dispose();
                //cmd.Transaction = new OleDbTransaction();

                HisConnOfOLEs.PutPool(connofora);
                return i;
            }
            catch (System.Exception ex)
            {
        ...
        {
            ConnOfOLE connofObj = GetAppCenterDBConn as ConnOfOLE;
            connofObj.AddSQL(selectSQL);
           // OleDbConnection conn = connofObj.Conn;
            ...
            try
            {
                ...
                ada.Fill(oratb);
                ada.Dispose();
                HisConnOfOLEs.PutPool(connofObj);
                return oratb;
            }
            catch (System.Exception ex)
            {
                HisConnOfOLEs.PutPool(connofObj);
                string msg = "@运行查询在(RunSQLReturnTable_200705_OLE with paras)出错 sql=" + selectSQL + " @异常信息：" + ex.Message;

	...
	public class DDL:System.Web.UI.WebControls.DropDownList
	{
        ...
        public void BindSysEnum(string key, bool isShowKey, AddAllLocation alllocal)
        {
            ...
            foreach (SysEnum en in ens)
            {
                ...
                {
                    ListItem li = new ListItem(en.Lab + en.Style, en.IntKey.ToString());
        ...
        {
            this.ID = "DDL_" + attr.Key;
           // this.AppPath = BP.SystemConfig.AppName;
            this.BorderStyle = BorderStyle.None;
        ...
        public void LoadMapAttr(MapAttr attr)
        {
            ...
            {
                this.SelfShowType = DDLShowType.Ens;
                this.HisFKEns = DA.ClassFactory.GetEns(attr.UIBindKey );
...
#warning 去了在2009-03-09
			...
			if (ens.IsGradeEntities)
			{
				...
				foreach(Entity en in ens)
				{
					...
					{
						blank="";
						blank=blank.PadLeft( en.GetValIntByKey("Grade")-1, '@' );
						blank=blank.Replace("@","_");
					}
					catch 
					{
						//throw new Exception("error Grade="+en.GetValIntByKey("Grade")+" No="+en.GetValStringByKey("No")+" Name"+en.GetValStringByKey("Name")+" Exception="+ex.Message );
					}
					this.Items.Add(new ListItem(blank+ en.GetValStringByKey(refText),  en.GetValStringByKey(refKey)  )) ; 
				}
			}
			else
			{

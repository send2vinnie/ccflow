...
namespace BP.Rpt.RTF
{
	...
	public class RTFEngine 
	{
        ...
        public void MakeDoc002_del(string cfile, string path, string file, bool isOpen)
        {
            string str = Cash.GetBillStr(cfile,false);
            this.TempFilePath = path + file;
            try
            {
                string key = "";
            ...
            {
                throw new Exception("生成文档失败：单据名称[" + this.CyclostyleFilePath + "] 异常信息：" + ex.Message);
            }

                ...
                if (this._en == null)
                {
                    ...
                    switch (map.EnDBUrl.DBType)
                    {
                        case DBType.Oracle9i:
                            ...
                            {
                                if (top == 0)
                                    sql = "SELECT * FROM ( SELECT  " + pk + " FROM " + map.PhysicsTable + "  order by " + attr2Field(orderBy) + isDescStr + "   ) WHERE ROWNUM <=" + pageSize;
                                else
                                    sql = "SELECT * FROM ( SELECT  " + pk + " FROM " + map.PhysicsTable + "  order by " +  attr2Field(orderBy) + isDescStr + ") ";
                            }
                            else
                            {
                                if (top == 0)
                                    sql = "SELECT * FROM ( SELECT  " + pk + " FROM " + map.PhysicsTable + " WHERE " + this._sql + " order by " + attr2Field(orderBy) + isDescStr + "   )  WHERE ROWNUM <=" + pageSize;
                                else
                                    sql = "SELECT * FROM ( SELECT  " + pk + " FROM " + map.PhysicsTable + " WHERE " + this._sql + " order by " + attr2Field(orderBy) + isDescStr + "   ) ";
                            }
                        ...
                        default:
                            ...
                            {
                                if (top == 0)
                                    sql = " SELECT top " + pageSize + "  [" + this.En.PKField + "] FROM " + map.PhysicsTable + "  order by " + attr2Field(orderBy) + isDescStr;
                                else
                            ...
                            {
                                if (top == 0)
                                    sql = "SELECT TOP " + pageSize + " [" + this.En.PKField + "] FROM " + map.PhysicsTable + " WHERE " + this._sql + " order by " + attr2Field(orderBy) + isDescStr;
                                else

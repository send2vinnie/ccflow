    ...
    */
    ...
    __OnMouseDown: function (e) {
        ...
        if (t) {
            ...
            var sf = this;
            if (this.allowDrag && panel.allowDrag && mini.isAncestor(panel.headerEl, e.target) && !mini.findParent(e.target, "mini-tools")) {
                var box = panel.getBox();
                var drag = new mini.Drag({
                    ...
                    onStart: function () {
                        ...
                        sf.dragProxy = panel.el.previousSibling;
                        //mini.setHeight(sf.dragProxy, box.height);
                        sf.dragProxy.style.height = box.height + "px";
                    ...
                    onMove: function (drag) {
                        ...
                        var dragBox = mini.getBox(sf.dragProxy);
                        dragBox.height += 10;
                        dragBox.bottom += 10;
                        ...
                        if (mini.isNumber(sf._targetColumn)) {
                            if (sf._targetPanel) {
                                ...
                                if (sf._targetPanel.__moveAction == "before") {
                                    jQuery(el).before(sf.dragProxy);
                                } else {
                                    jQuery(el).after(sf.dragProxy);
                                }
                            } else {
                                var el = sf.getColumnEl(sf._targetColumn);
                                mini.append(el, sf.dragProxy);
                            }
                        }
                    },
                    onStop: function () {
                        //从dragProxy，找column和index
                        var td = sf.dragProxy.parentNode;
                        var column = parseInt(td.id);

                        jQuery(sf.dragProxy).before(panel.el);

                        sf.panels.remove(panel);

                        var next = sf.dragProxy.nextSibling;
                        if (!next) {

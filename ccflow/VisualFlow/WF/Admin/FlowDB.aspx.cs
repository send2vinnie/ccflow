...
public partial class WF_Admin_FlowDB : BP.Web.WebPage
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        foreach (GenerWorkFlow item in gwfs)
        {
            ...
            this.Pub1.AddTD(item.RecName);
            this.Pub1.AddTD(item.RDT);
            this.Pub1.AddTD(item.NodeName);
            this.Pub1.AddTDB(item.Title);
            this.Pub1.AddTDBegin();
            this.Pub1.Add("<a href=\"javascript:WinOpen('./../Chart.aspx?WorkID=" + item.WorkID + "&FK_Flow=" + this.FK_Flow + "&FID=" + item.FID + "','ds'); \" >轨迹图</a>-");
            this.Pub1.Add("<a href=\"javascript:WinOpen('./../WFRpt.aspx?WorkID=" + item.WorkID + "&FK_Flow=" + this.FK_Flow + "&FID=" + item.FID + "','ds'); \" >报告</a>-");
            this.Pub1.Add("<a href=\"javascript:DelIt('" + item.FK_Flow + "','" + item.WorkID + "');\"><img src='./../../Images/Btn/Delete.gif' border=0/>删除</a>");
            this.Pub1.AddTDEnd();
            this.Pub1.AddTREnd();
        }

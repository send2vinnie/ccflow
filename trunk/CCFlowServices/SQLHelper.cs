...
namespace MyService.Common
{
	...
	public sealed class SqlHelper
	{
		...
		static SqlHelper()
		{
            _connectionString = ""; // AppUtil.GetDBCfg().GetProperty("");
		}
		...
		#region GetDataSet
		public static DataSet GetDataSet(SqlDataAdapter sda)
		{

			System.Data.SqlClient.SqlConnection g_sqlConnection = new SqlConnection(_connectionString);
			try
			{
				if (sda.SelectCommand.Connection ==null || sda.SelectCommand.Connection.State != ConnectionState.Open)
				{
					sda.SelectCommand.Connection=g_sqlConnection;				
			...
			{
				if (g_sqlConnection.State == ConnectionState.Open)
		...
		public static DataSet GetDataSet(string SqlQuery)
		{
			...
			{
				g_sqlConnection.Open();
				SqlCommand sqlCMD = new SqlCommand(SqlQuery,g_sqlConnection);
			...
			finally
			{
				try
				{
					if (g_sqlConnection.State != ConnectionState.Closed)
						g_sqlConnection.Close();

					g_sqlConnection = null;
				}
				catch{}
			}
		}

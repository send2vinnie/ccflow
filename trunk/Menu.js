...
/*编辑一对多的关系*/
function EditOneVsM1( path, vsMName , attrKey, ensName, keys )
{
   var url=path+'/Comm/'+'UIEn1ToM.aspx?EnsName='+ensName+"&AttrKey="+attrKey+keys;
  // alert (url ) ;
   var a=window.showModalDialog( url , 'OneVs' ,'dialogHeight: 600px; dialogWidth: 800px; dialogTop: 100px; dialogLeft: 110px; center: yes; help: no'); 
   return true;
}
...
/*编辑明晰 */
function EditDtl1(path, ensName , dtlName, refKey, enKeys)
{
 if (ensName==null)
      return ;
   var newWindow=window.open( path+'/Comm/UIEnDtl.aspx?EnsName='+dtlName+"&RefKey="+refKey+"&MainEnsName="+ ensName +enKeys,'chosecol', 'width=700,top=100,left=200,height=400,scrollbars=yes,resizable=yes,toolbar=false,location=false' );
   newWindow.focus();
   return true;
}
...
{
   var newWindow=window.open( 'PanelEns.aspx?EnsName='+className+'&GroupKey='+key, 'mainfrm', 'width=900,top=60,left=60,height=500,scrollbars=yes,resizable=yes,toolbar=false,location=false' );
}
...
function GroupEns( className )
{
 if (CurrEnsName==null)
   ...
   var newWindow=window.open( WebPath+'/Comm/GroupEnsMNum.aspx?EnsName='+className ,'mainfrm', 'width=900,top=60,left=60,height=500,scrollbars=yes,resizable=yes,toolbar=false,location=false' );
   return true;
}

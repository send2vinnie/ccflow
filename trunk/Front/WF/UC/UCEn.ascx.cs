...
namespace BP.Web.Comm.UC
{
    ...
    public partial class UCEn : BP.Web.UC.UCBase3
    {
                ...
                else if (attr.UIContralType == UIContralType.DDL)
                {
                    if (isReadonly || !attr.UIIsReadonly)
                    {
                        ...
                        if (attr.MyFieldType == FieldType.Enum)
                        {
                            ...
                            BP.Sys.SysEnum enEnum = new BP.Sys.SysEnum(attr.UIBindKey, "CN", enumKey);
        ...
        public void InsertCellsData(MapData md, Entity en, string groupID, int groupIdx)
        {
            ...
            BP.Sys.DataCells dcs = null;

            if (en.IsBlank)
                dcs = new DataCells();
            else
                dcs = new DataCells(md.No + "_" + en.PKVal.ToString());

            while (dcs.Count < dcs.Count)
            {
                dcs.AddEntity(new Sys.DataCell());
            }

            if (isReadonlyTable == false)
               dcs.AddEntity(new Sys.DataCell());

            int idx = 0;
            foreach (DataCell dc in dcs)
            {
                idx++;
                this.AddTR();
                foreach (MapAttr attr in attrs1)
                {
                    ...
                    string val =  dc.GetValStrByKey(attr.KeyOfEn);
                    switch (attr.UIContralType)
                    {
                        case UIContralType.TB:
                            TB tb = new TB();
                            tb.ID = "TB_" + attr.KeyOfEn + "_" + idx;
                            tb.Text = val;
                        ...
                        case UIContralType.DDL:
                            ...
                            {
                                DDL ddl = new DDL();
                                ddl.ID = "DDL_" + attr.KeyOfEn+"_"+idx;
                            ...
                            {
                                /* 可以使用的情况. */
                                DDL ddl1 = new DDL(attr.HisAttr, val.ToString(), "enumLab", true, this.Page.Request.ApplicationPath);
                                ddl1.ID = "DDL_" + attr.KeyOfEn + "_" + idx;
                                this.AddTD(ddl1);
                            }
                            break;
        ...
        public void SaveDataCells(MapData md, string refval)
        {
            DataCells dcs = new DataCells(md.No + "_" + refval);
            MapAttrs attrs = md.GenerHisTableCells;
            for (int y = 1; y <= 80; y++)
            {
                if (this.IsExit("TB_F1_" + y) == false)
                    break;
                ...
                foreach (MapAttr attr in attrs)
                {
                    ...
                    dc.SetValByKey(attr.KeyOfEn, this.GetTBByID("TB_" + attr.KeyOfEn + "_" + y).Text);
                }
                if (dc.OID == 0)
                    dcs.AddEntity(dc);
            }
            ...
            foreach (DataCell dc in dcs)
            {
                ...
                if (dc.OID == 0 && dc.IsBlank)
                    continue;

                dc.RefVal = md.No + "_" + refval;

                string s = "";
                for (int i = 0; i < 20; i++)
                {
                    s += dc.GetValStrByKey("F" + i.ToString());
                }
        ...
        public void Bind(Entity en, string enName, bool isReadonly, bool isShowDtl, string noKey)
        {
            ...
            int GroupIdx = 0;
            this.Controls.Clear();
            this.Add("<Table border=1 class='' >");
            if (cf.GroupTitle.Length > 3)
            {
                this.AddTR("onclick=\"GroupBarClick('ND');\" ");
                this.AddTD("colspan=4 class=FDesc width='600px' ", "&nbsp;&nbsp;<img src='./Style/Max.gif' alert='Max' id='ImgND'  border=0 />&nbsp;<b>" + en.EnDesc+ "</b>");
                this.AddTREnd();
            }
            else
            {
                this.Add("<TR>");
                this.AddTD("colspan=4 class=FDesc width='600px' ", "&nbsp;&nbsp;<b>" + en.EnDesc + "</b>");
                this.Add("</TR>");
            }
            ...
            foreach (Attr attr in attrs)
            {
                ...
                foreach (string g in gTitles)
                {
                    if (g.Contains(attr.Key + "="))
                    {
                        ...
                        GroupKey = ss[0];
                        GroupIdx = 0;
                        this.AddTR("onclick=\"GroupBarClick('" + ss[0] + "');\" ");
                        this.AddTD("colspan=4 class=FDesc ", "&nbsp;&nbsp;<img src='./Style/Max.gif' alert='Max' id='Img" + ss[0] + "'  border=0 />&nbsp;<b>" + ss[1] + "</b>");
                        this.AddTREnd();
                        isLeft = true;
                        break;
                    }
                ...
                if (attr.UIIsLine)
                {
                    ...
                    this.AddTR(" ID='"+GroupKey+GroupIdx+"'");
                    GroupIdx++;
                    ...
                    if (attr.UIHeight != 0)
                    {
                        ...
                        else
                        {
                            ...
                            EditerType type = EditerType.None; // = (EditerType)EnsAppCfgs.GetValInt(en.ToString() + "s", "EditerType");
                            switch (type)
                            {
                                ...
                                case EditerType.Sina:
                                  ...
                                  //  input.Attributes["id"] = "TBH_"+attr.Key;
                                    ...
                                    this.Add("<td class='FDesc'  colspan='4' nowrap height='100px' width='50%'>");
                                    this.Add(input);
                                    this.Add("<iframe ID='eWebEditor1' src='./../Comm/Ctrl/editor/editor.htm?id=" + input.ClientID + "&style=coolblue' frameborder='0' scrolling='no' width='600' HEIGHT='350'></iframe>");
                                    this.AddTDEnd();
                                    break;
                                default:
                                    TB area = new TB();
                    ...
                    switch (attr.UIContralType)
                    {
                        case UIContralType.TB:
                            ...
                            if (attr.Key == noKey && attr.UIIsReadonly == false && attr.UIVisible == true)
                            {
                                string path = this.Request.ApplicationPath;
                                //string helpScript = "";
                                string helpScript = "<a href=\"javascript:alert('请在文本框上双击，系统会出现查找窗口。');\" ><img src='/Images/Btn/Search.gif' border=0 /></a>";
                            ...
                            switch (type)
                            {
                                case EditerType.None:
                                    ...
                                    if (attr.UIIsReadonly || isReadonly)
                                    {
                                        ...
                                        areaR.TextMode = TextBoxMode.MultiLine;
                                        areaR.ReadOnly = true;
                                        this.AddContral(attr.Desc, areaR);
                                    }
                                    else
                                    {
                                        TB area = new TB();
                                ...
                                case EditerType.FKEditer:
                                    ...
                                    //input.ID = attr.Key;
                                    this.Add("<td class='FDesc'  colspan='2' nowrap height='100px' width='50%' >" + attr.Desc + "<br>");
                                    this.Add(input);
                        ...
                        else
                        {
                            ...
                            switch (attr.MyDataType)
                            {
                                ...
                                case DataType.AppDateTime:
                                    ...
                                    //tb.Attributes["onfocus"] = "new WdatePicker(this,'%Y-%M-%D %h:%m',true,'"+tb.Text+"')";
                                    // tb.Attributes["onfocus"] = "new WdatePicker(this,'%Y-%M-%D %h:%m',true,'default')";
                                    break;
                                default:
                                    tb.Text = val.ToString();
                                    tb.Attributes["Class"] = "TB";
                            ...
                            if (attr.Key == noKey && attr.UIIsReadonly == false && attr.UIVisible == true)
                            {
                                ...
                                string helpScript = "<a href=\"javascript:alert('请在文本框上双击，系统会出现查找窗口。');\" ><img src='/Images/Btn/Search.gif' border=0 /></a>";
                                tb.Attributes["ondblclick"] = "OpenHelperTBNo('" + path + "','BP.Port.Taxpayers',this)";
                                this.AddContral(attr.Desc, tb, helpScript);
                            }
                            else
                            {
                                this.AddContral(attr.Desc, tb);
            ...
            if (num > 0 && en.IsExit(en.PK, en.PKVal) == false)
            {
                ...
                if (endMsg != "")
                {
                    this.Add("<TR ><TD class=FDesc rowspan=4 >");
                    this.Add(endMsg);
                    this.Add("</TD></TR>");
            ...
            if (refstrs != "")
            {
                this.Add("<font style='font-size:14px' >" + refstrs + "</font>");
                // this.Add("<HR><table class=Table border=0 ><TR class=TR ><TD class=TD ><font style='font-size:14px' >" + refstrs + "</font></TD></TR></table>");
            }
        }

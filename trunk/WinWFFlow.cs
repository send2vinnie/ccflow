...
namespace BP.Win.WF
{
    ...
    public class WinWFFlow : BP.Win32.Controls.WFContainer // System.Windows.Forms.Panel
	{
		...
		public Flow HisFlow = null;     //工作流    数据实体
		...
		public override void Save()
		{
            ...
            //    catch (Exception ex)
            //    {
            //        str += "@获取 work error " + ex.Message;
            //        continue;
            //    }
            //    if (wk.EnMap.Attrs.Contains("FID") == false)
            //    {
            //        BP.Sys.MapAttr attr = new BP.Sys.MapAttr();
            //        attr.FK_MapData = "ND" + nd.NodeID;
            //        attr.IsCanDel = false;
            //        attr.IsCanEdit = false;
            //        attr.KeyOfEn = "FID";
            //        attr.Name = "FID";
        ...
        public void SaveHisLabNotes()
        {
            foreach (Control con in this.Controls)
            {
                ...
                if (node != null && node.HisNode != null && node.HisNode.OID > 0)
                {
                    node.HisNode.X = node.Location.X - this.DisplayRectangle.X;
                    node.HisNode.Y = node.Location.Y - this.DisplayRectangle.Y;
                    node.HisNode.DirectUpdate();
...
#warning 去了节点完成条件。
        ...
        private void AddNode(Point loc)
        {
            ...
            int idx = this.NodeCount;
            while (true)
            {
                string strID = this.HisFlow.No.Trim() + idx.ToString().PadLeft(2, '0');
            ...
            if (Global.ToolIndex == 2)
                nd.HisNodeWorkType = NodeWorkType.GECheckStands;
            else
            ...
            nd.FK_Flow = this.HisFlow.No;
            nd.FlowName = this.HisFlow.Name;
            nd.X = loc.X;
            nd.Y = loc.Y;
            nd.Step = idx;
            nd.Save();
            node.HisNode = nd;
            if (nd.HisNodeWorkType == NodeWorkType.GECheckStands )
            {
                node.Text = this.ToE("CheckNode", "审核节点") + idx; //审核节点
            }
            else
            {
                node.Text = this.ToE("OrdinaryNode", "普通工作节点") + idx; //普通工作节点
            }
            this.Controls.Add(node);
            this.NodeCount++;
            ...
            if (Global._copyNode == null)
                return;

            BP.Sys.MapData currMD = new BP.Sys.MapData();
            currMD.No = "ND" + node.HisNode.NodeID;
            if (currMD.IsExits == true)
                return;

            BP.Sys.MapData md = new BP.Sys.MapData("ND" + Global._copyNode.NodeID);
            md.Copy(currMD);
            md.Name = node.HisNode.Name;
            md.No = "ND" + node.HisNode.NodeID;
            md.Insert();

            BP.Sys.MapAttrs attrs = new BP.Sys.MapAttrs(md.No);
            foreach (BP.Sys.MapAttr attr in attrs)
            {
                BP.Sys.MapAttr myattr = new BP.Sys.MapAttr();
                myattr.Copy(attr);
                myattr.FK_MapData = currMD.No;
                myattr.OID = 0;
                myattr.Insert();
            }

            Global._copyNode = null;
        }
		/// <summary>
		/// 增加一个节点
		/// </summary>
		/// <param name="wfn"></param>
		private void AddNode(Node wfn)
		{
        ...
        {
            if (!this.FindNode(wfn.OID.ToString()))
            {
        ...
        private void LabNote_Click(object sender, System.EventArgs e)
        {
            ...
            if (lab != null)
            {
                BP.WF.Global.DoEdit(lab.HisNode);
                return;
            }
            ...
            switch (mi.Name )
            {
                ...
                default:
                    this.SaveHisLabNotes();
                    BP.WF.Global.DoEdit(nd.HisNode);
        ...
        private void DoNodeDel_Click(object sender, System.EventArgs e)
        {
            ...
            {
                MessageBox.Show(this.ToE("NoDelStartNode", "您不能删除开始节点"), "Error", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }

            if (MSG.ShowQuestion( "提示",this.ToE("AYS", "您确定吗？") ) == DialogResult.No)
                return;
            ...
            {
                if (MSG.ShowQuestion("节点" + node.HisNode.Name + "[" + node.HisNode.NodeID + "]删除失败！：" + ex.Message + "\n是否强行从数据库中删除该节点？", "删除失败！") == DialogResult.Yes)
                {
		...
		private void miWorkP_Click(object sender, System.EventArgs e)
		{
            ...
            if (node != null)
            {
                ...
                //if (node.HisNode.IsCheckNode)
                //{
                //    BP.Win32.PubClass.Alert("审核节点不能设表单。");
                //    return;
                //}

                node.HisNode.CreateMap();
                BP.WF.Global.DoUrl("/DoPort.aspx?DoType=MapDef&PK=ND" + node.HisNode.NodeID);
                return;

                //if( fattr.ShowAttr("工作属性 ["+ node.HisNode.Name +"]", node.HisNode.HisWork ,edit) ==DialogResult.OK)
                //	node.BindWFNode();
            }
		}
		...
		private void miNodeP_Click(object sender, System.EventArgs e)
		{
            ...
            if (node != null)
            {
                ...
                //BP.WF.Design.Global.DoIE("Node", node.HisNode.NodeID);
                //return;
                //System.Diagnostics.Process.Start("IExplore.exe", "http://localhost/Front/Comm/UIEn.aspx?EnName=BP.WF.Node&PK="+node.HisNode.NodeID );
                //return;
                ...
                Flow fl = new Flow(this.HisFlow.No);
                node.HisNode.FlowName = fl.Name;
                node.HisNode.Update();
        ...
        private void DoNodeStation_Click_del(object sender, System.EventArgs e)
        {
            MenuItem mi = sender as MenuItem;
            ContextMenu menu = mi.Parent as ContextMenu;
            WinWFNode node = menu.SourceControl as WinWFNode;
            ...
            fotm.lab2.Text = this.ToE("Station","岗位"); 

            fotm.SetBindKey( this.ToE("NodeStation","节点岗位") 
        ...
        private void DoNodeDept_Click(object sender, System.EventArgs e)
        {
            MenuItem mi = sender as MenuItem;
            ContextMenu menu = mi.Parent as ContextMenu;
            WinWFNode node = menu.SourceControl as WinWFNode;
            ...
            FrmOneToMore fotm = new FrmOneToMore();
            fotm.lab1.Text = this.ToE("Node", "节点");
            fotm.lab2.Text = this.ToE("Station", "部门");

            fotm.SetBindKey(this.ToE("NodeDept", "节点部门")
                , NodeAttr.NodeID, false
                , "No", false
            ...
            Depts ens2 = new Depts();
            ens2.RetrieveAll();
            fotm.FillEnsAndEns(ns, NodeAttr.Name, false, node.HisNode
                , ens2, "Name");

            NodeDepts nds = new NodeDepts(node.HisNode.NodeID);
            fotm.BindCheckEns(nds);

            fotm.tree2.DoubleClick += new EventHandler(this.tree2_DoubleClick);
           // fotm.btnNext.Text = this.ToE("EmpDept", "岗位工作人员");  // "岗位工作人员";
           // fotm.btnNext.Show();
		...
		private void tree2_DoubleClick(object sender,EventArgs e)
		{
			Tree tr = sender as Tree ;
            if (tr == null)
            {
                FrmOneToMore fotm = ((Control)sender).FindForm() as FrmOneToMore;
                if (fotm != null)
                    tr = fotm.tree2;
            }


			if(tr ==null || tr.SelectedNode==null || tr.SelectedNode.Tag == null )
				return;

			Station source =  tr.SelectedNode.Tag as Station;//new Station( no );
            if (source != null)
            {
                FrmOneToMore fotm = new FrmOneToMore();
                fotm.lab1.Text = this.ToE("Station", "岗位");
                fotm.lab2.Text = this.ToE("Emp", "操作员");
                fotm.ReadOnly = true;

                fotm.FillTree2ByEmps();

                Stations ens = new Stations();
                ens.RetrieveAll();
                fotm.FillTree1ByEns(ens, NodeAttr.Name, false, source);

                EmpStations nds = new EmpStations(source.No);
                fotm.BindCheckEns(nds);
		...
		private void miLineDirCondition_Click(object sender, System.EventArgs e)
		{
            ...
            {
              //  FrmCondition fotm = new FrmCondition();
                ...
                //  BP.WF.Global.DoUrl("./DoPort.aspx?DoType=Dir&ToNodeID=" + conn.ToNodeID + "&FK_Flow=" + this.HisFlow.No + "&FK_MainNode=" + conn.NodeID + "&FK_Node=" + conn.FK_Node + "&FK_Attr=" + conn.FK_Attr + "&AttrKey=" + conn.AttrKey + "&DirType=BP.WF.DirectionCondition");
                return;

                //Directions dirs = new Directions(line.HisDirection.Node);
                //if (dirs.Count == 1)
                //{
                //    //  MessageBox.Show("没有分支，您不必设置方向条件。","提示",MessageBoxButtons.OK,MessageBoxIcon.Information);
                //    //  BP.Win32.PubClass.Alert("");
                //    //return;
                //    fotm.ShowFrm(line.HisDirection.Node, "方向条件 － [" + line.Text + "]", conn);
                //}
                //else
                //    fotm.ShowFrm(line.HisDirection.Node, "方向条件 － [" + line.Text + "]", conn);
            }

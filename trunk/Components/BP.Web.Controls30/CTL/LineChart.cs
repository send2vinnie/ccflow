...
namespace  BP.Web.Controls
{
	...
	public class LineChart : System.Web.UI.WebControls.Image
	{
		...
		public void Draw() 
		{
			...
			#region 沿X坐标写入X标签.
			YearMonths ym = this.ChartYearMonths;			 
			for (   i = 0 ; i <= ym.Count-1 ; i++ )  ///循环每一个 yearmonth ., 
			{
				x = ChartInset + ( i * ChartWidth ) / ym.Count-1;
				y = ChartHeight + ChartInset;
				g.DrawString(ym[i].strYearMonth , axesFont , BlackBrush , x-4 , y +10 ) ; /// 输出一个图标。标年度　。
				g.DrawLine(BlackPen , x , y+2   , x , y-2); /// 让其显示一个 dot 在图上。 有两个基本很近的点组合成的。
				ym[i].x=x;
				ym[i].y=y;

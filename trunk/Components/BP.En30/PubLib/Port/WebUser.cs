    ...
    public class WebUser
    {
        ...
        public static void SignInOfGener(Emp em, string lang, string auth, bool isRememberMe)
        {
            ...
            if (BP.SystemConfig.IsBSsystem)
            {
                ...
                //  cookie.Values.Add("FK_DeptName", em.FK_DeptText);
        ...
        public static string GetSessionByKey(string key, string isNullAsVal)
        {
            if (IsBSMode)
            {
                ...
                {
                    System.Web.HttpContext.Current.Session[key] = isNullAsVal;
                    return isNullAsVal;
                }
                else
                {
            ...
            get
            {
                ...
                if (s == null)
                {
                    if (WebUser.No == null)
                        ...
                        return GetSessionByKey("FK_Dept", null);
                }
                return s;
            }
            set
            ...
            get
            {
                ...
                if (no == null || no == "")
                {
                    if (IsBSMode)
                    {
                        ...
                        {
                            //string s = hc["IsRememberMe"] as string;
                            //if (s == null || s == "0" || s == "")
                            //    return null;

                            WebUser.No = hc["No"];

                            string name=BP.DA.DBAccess.RunSQLReturnStringIsNull("SELECT Name FROM Port_Emp where No='" + HttpUtility.UrlDecode(hc["No"] + "'"),"null");
                            if (name == "null")
                                return null;

                             WebUser.SetSessionByKey("Name",name);

                           // WebUser.Name = HttpUtility.UrlDecode(hc["Name"], System.Text.Encoding.UTF32);
                        ...
                        //  System.Web.HttpContext.Current.Response.Redirect(SystemConfig.PageOfLostSession, true);
                    }
                    else
                    {
                        return "admin";
        ...
        public static Depts HisDepts
        {
            get
            {
                ...
                if (obj == null)
                {
                    Depts sts = new Depts();
                    QueryObject qo = new QueryObject(sts);
                    qo.AddWhereInSQL("No", "SELECT FK_Dept FROM Port_EmpDept WHERE FK_Emp='" + WebUser.No + "'");
                    qo.DoQuery();
        ...
        /// 提示声音
        /// </summary>
        public static string NoticeSound
        {
            get
            {
                return (string)GetSessionByKey("NoticeSound", false);

            }
            set
            {
                SetSessionByKey("NoticeSound", value);
            }
        }
        /// <summary>
        /// 最后登录日期
        ...
        /// 是否是授权状态
        /// </summary> 
        public static bool IsTurnTo_del
        {
            get
            {
                return (bool)GetSessionByKey("IsTurnTo", false);

            }
            set
            {
                SetSessionByKey("IsTurnTo", value);
        ...
        /// 声音提示
        /// </summary>
        public static bool IsSoundAlert
        {
            get
            {
                return (bool)GetSessionByKey("IsSoundAlert", false);
            }
            set
            {
                SetSessionByKey("IsSoundAlert", value);
            }
        }
        /// <summary>
        /// 是不是文字提示
        /// </summary>
        public static bool IsTextAlert
        {
            get
            {
                return (bool)GetSessionByKey("IsTextAlert", false);
            }
            set
            {
                SetSessionByKey("IsTextAlert", value);
            }
        }

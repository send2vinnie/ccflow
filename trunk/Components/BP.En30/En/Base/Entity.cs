...
namespace BP.En
{
    ...
    abstract public class Entity : EnObj
    {
        ...
        public string GenerNewNoByKey(int nolength, string attrKey, string attrGroupKey, string attrGroupVal)
        {
            ...
            {
                case DBType.SQL2000:
                    sql = "SELECT CONVERT(INT, MAX([" + field + "]) )+1 AS No FROM " + this.EnMap.PhysicsTable + " WHERE " + attrGroupKey + "='" + attrGroupVal + "'";
                    break;
                case DBType.Access:
        ...
        public virtual void Insert()
        {
            ...
            try
            {
                ...
                {
                    case DBType.Access:
                        this.RunSQL(SqlBuilder.InsertOFOLE(this));
                        break;
                    default:
                        this.RunSQL(this.SQLCash.Insert, SqlBuilder.GenerParas(this, null));
                        break;
        ...
        public void CheckPhysicsTable()
        {
            ...
            #region 建立主键
            if (this.EnMap.EnType != EnType.View && DBAccess.IsExitsTabPK(this.EnMap.PhysicsTable) == false)
            {
                try
                {
            ...
            foreach (Attr attr in this.EnMap.Attrs)
            {
                ...
                if (DBAccess.IsExitsTableCol(this.EnMap.PhysicsTable, attr.Field) == false)
                {
                    if (attr.Key == "AID")
                    {
                        /* 自动增长列 */
                    ...
                    switch (attr.MyDataType)
                    {
                        ...
                        case DataType.AppDateTime:
                            ...
                            if (dbtype == DBType.Access && len > 1000)
    ...
    public abstract class Entities : CollectionBase
    {
        ...
        public Entity GetEntityByKey(string attr1, object val1, string attr2, object val2)
        {
            foreach (Entity en in this)
            {
                if (en.GetValByKey(attr1).Equals(val1) && en.GetValByKey(attr2).Equals(val2))
                    return en;
            }
            return null;
        }

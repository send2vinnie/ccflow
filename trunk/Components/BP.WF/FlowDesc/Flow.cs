        ...
        public Work NewWork()
        {
            ...
            else
            {
                ...
                try
                {
                    ...
                    #region 根据lijian 的反馈出现多次 workid 重复的问题.
                    if (num != 0)
                    {
                        // 检查流程注册表时是否有此工作OID.
                        int currNodeID = DBAccess.RunSQLReturnValInt("SELECT FK_Node FROM WF_GenerWorkFlow WHERE WorkID=" + wk.OID, 0);
                        if (currNodeID != 0)
                        {
                            /* 如果有， 就判断当前节点是否是开始节点 */
                            if (currNodeID == int.Parse(this.No + "01"))
                            {
                                // throw new Exception(msg);
                            }
                            else
                            {
                                string msg = "@新建工作出现错误，请把问题反馈给管理员，谢谢。技术信息:流程编号:" + this.No + ",名称:" + this.Name;
                                msg += "WorkID" + wk.OID + "出现了重复情况。";
                                Log.DefaultLogWriteLine(LogType.Error, "出现WorkID=" + wk.OID + "重复,不应该出现的问题. fk_flow:" + this.No + " Name=" + this.Name);
                                num = 0; /* 让它新建一个work */
                            }
                        }
                        else
                        {
                            /*有可能流程结束了，还是启用这个WorkID */
                            int currWFState = DBAccess.RunSQLReturnValInt("SELECT WFState FROM ND" + int.Parse(this.No) + "Rpt WHERE OID=" + wk.OID, 0);
                            if (currWFState != 0)
                            {
                                string msg = "@新建工作出现错误，请把问题反馈给管理员，谢谢。技术信息:流程编号:" + this.No + ",名称:" + this.Name;
                                msg += "WorkID" + wk.OID + "出现了重复情况。";
                    ...
                    #endregion 根据lijian 的反馈出现多次 workid 重复的问题.
                }
                catch
                {
                    wk.CheckPhysicsTable();
                ...
                if (num == 0)
                {
                    ...
                    #region 根据lijian 的反馈出现多次workid 重复的问题.
                    if (DBAccess.RunSQLReturnValInt("SELECT COUNT(*) FROM ND"+int.Parse(this.No)+"Rpt WHERE OID="+wk.OID) == 1)
                    {
                        Log.DefaultLogWriteLine(LogType.Error, "出现WorkID重复,不应该出现的问题.");
                        throw new Exception("@新建工作出现错误，请把问题反馈给管理员，谢谢。");
                    }
                    #endregion
...
#warning 不应该出现的工作丢失.
        ...
        public string DoCheck()
        {
            ...
            string msg = "<font color=blue>关于《" + this.Name + " 》 流程检查信息</font><hr>";
            Nodes nds = new Nodes(this.No);
            Emps emps = new Emps();

            string rpt = "<html>\r\n<title>" + this.Name + "-流程诊断报告</title>";
            rpt += "\r\n<body>\r\n<table width='70%' align=center border=1>\r\n<tr>\r\n<td>";
            rpt += "\r\n<div aligen=center><h1><b>" + this.Name + " - 流程设计文档</b></h1></div>";
            rpt += "<b>流程设计:</b><br>";
            rpt += "&nbsp;&nbsp;" + this.Note;
            rpt += "\r\n<br><b>信息:</b><br>";
            ...
            rpt += "&nbsp;&nbsp;步骤:" + nds.Count + "，" + "单据" + ":" + bks.Count + "个。";
                ...
                switch (nd.HisDeliveryWay)
                {
                    ...
                    case DeliveryWay.BySpecNodeEmpStation: /*按指定的岗位计算.*/
                        ...
                        else
                        {
                            ...
                            {
                                rpt += "<font color=red>您没有设置指定岗位的节点编号，目前设置的为{" + nd.DeliveryParas + "}</font>";
                                //Node mynd = new Node(int.Parse(nd.DeliveryParas));
                    ...
                    case DeliveryWay.BySQL:
                        ...
                        else
                        {
                            try
                            {
                                DataTable testDB = DBAccess.RunSQLReturnTable(nd.DeliveryParas);
                    ...
                    case DeliveryWay.ByPreviousNodeFormEmpsField:
                        if (attrs.Contains(BP.Sys.MapAttrAttr.KeyOfEn, nd.DeliveryParas) == false)
                        {
                            /*检查节点字段是否有FK_Emp字段*/
            ...
            msg += "<br><a href='/DataUser/FlowDesc/" + this.Name + ".htm' target=_s>设计报告</a>";
        ...
        private void GenerWorkTempleteDoc(Entity en, string enName, Node nd)
        {
            ...
            try
            {
                ...
                WordApp.ActiveWindow.ActivePane.Selection.InsertAfter("模板由ccflow5自动生成，严谨转载。欢迎使用开源,免费的工作流程管理系统 http://ccFlow.org.");
...
#warning 没有处理好mysql的删除。
        ...
        public override Map EnMap
        {
            get
            {
                ...
                //参数.
                map.AddTBString(FlowAttr.Paras, null, "参数", true, false, 0, 300, 10);

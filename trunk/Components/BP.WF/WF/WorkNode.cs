...
#warning edit by peng, 用来确定不同岗位集合的传递包含同一个人的处理方式。
           ...
           while (true)
           {
               ...
               if (dt.Rows.Count == 0)
               {
                   ...
                   {
                       Dept deptP = new Dept(nowDeptID);
                       sql += "(SELECT FK_Emp FROM Port_EmpDept WHERE FK_Emp!=" + dbStr + "FK_Emp2 AND FK_Dept = '" + deptP.ParentNo+ "')";
                   }
       ...
       public bool IsSubFlowWorkNode
       {
        ...
        private WorkNode DoReturnSubFlow(int backtoNodeID, string msg, bool isHiden)
        {
            ...
            if (Glo.IsEnableSysMessage)
            {
                ...
                BP.Sys.SMS.AddMsg(wl.WorkID + "_" + wl.FK_Node + "_" + wn.HisWork.Rec + DateTime.Now.ToString("HHmmss"), wn.HisWork.Rec,
                                     title + msg, title, msg);
            }
            return wn;
        ...
        public WorkNode DoReturnWork(int backtoNodeID, string msg, bool isBackTracking)
        {
            ...
            if (Glo.IsEnableSysMessage == true)
            {
                ...
                BP.Sys.SMS.AddMsg(rw.MyPK, wnOfBackTo.HisWork.Rec, title + msg, title, msg);
            }
...
#warning 记录日志.
                ...
                if (this.HisNode.HisFormType != FormType.SDKForm)
                {
                    if (this.HisNode.IsStartNode)
                        ...
                        this.addMsg(SendReturnMsgFlag.ToEmps, "@<a href='" + this.VirPath + this.AppType + "/MyFlowInfo" + Glo.FromPageType + ".aspx?DoType=UnSend&WorkID=" + toWK.OID + "&FK_Flow=" + toND.FK_Flow + "'><img src='/WF/Img/UnDo.gif' border=0/>撤销本次发送</a>。");
                }
                ...
                {
                    this.addMsg(SendReturnMsgFlag.WorkRpt, null, "@<img src='/WF/Img/Btn/PrintWorkRpt.gif' ><a href='/WF/WFRpt.aspx?WorkID=" + toWK.OID + "&FID=" + toWK.FID + "&FK_Flow=" + toND.FK_Flow + "'target='_blank' >工作报告</a>。");
                }
                #endregion
...
#warning 为了不让其显示在途的工作需要， =3 不是正常的处理模式。
                ...
                if (Glo.IsEnableSysMessage)
                {
                    ...
                    foreach (Listen lt in lts)
                    {
                        ...
                        foreach (DataRow dr in dtRem.Rows)
                        {
                            ...
                            BP.Sys.SMS.AddMsg(lt.OID + "_" + this.WorkID, fk_emp, title + doc, title, doc);
                        }
                    }
                }
                #endregion
                    ...
                    try
                    {
                        ...
                        if (ccers.Rows.Count > 0)
                        {
                           ...
                           this.HisNode.FlowName, this.HisNode.Name, WebUser.Name);
                                ...
                                BP.Sys.SMS.AddMsg(list.RefWorkID + "_" + list.FK_Node + "_" + wfemp.No, wfemp.No,
                                  title, title, mytemp);
                            }
        ...
        public string FeiLiuStartUp(Node toNode)
        {
            ...
            if (this.HisNode.IsStartNode)
            {
                if (gwls.Count >= 2)
                {
                    this.addMsg(SendReturnMsgFlag.EditAccepter, null, "@<img src='/WF/Img/AllotTask.gif' border=0 /><a href=\"javascript:WinOpen('/WF/AllotTask.aspx?WorkID=" + this.WorkID + "&FID=" + this.WorkID + "&NodeID=" + toNode.NodeID + "')\" >修改接受对象</a>.", 
            ...
            if (this.HisNode.IsStartNode)
            {
                this.addMsg(SendReturnMsgFlag.NewFlowUnSend, null,
                    "@<a href='" + this.VirPath + this.AppType + "/MyFlowInfo" + Glo.FromPageType + ".aspx?DoType=UnSend&WorkID=" + this.WorkID + "&FK_Flow=" + toNode.FK_Flow + "'><img src='/WF/Img/UnDo.gif' border=0/>撤销本次发送</a>， <a href='" + this.VirPath +  this.AppType + "/MyFlow" + Glo.FromPageType + ".aspx?FK_Flow=" + toNode.FK_Flow + "&FK_Node=" + toNode.FK_Flow + "01' ><img src=" + this.VirPath + "WF/Img/New.gif border=0/>新建流程</a>。", 
                    SendReturnMsgType.Info);
            }
            else
            {
                this.addMsg(SendReturnMsgFlag.UnSend,null,
                    "@<a href='" + this.VirPath + this.AppType + "/MyFlowInfo" + Glo.FromPageType + ".aspx?DoType=UnSend&WorkID=" + this.WorkID + "&FK_Flow=" + toNode.FK_Flow + "'><img src='/WF/Img/UnDo.gif' border=0/>撤销本次发送</a>。", 
                    SendReturnMsgType.Info);
            }
           ...
           // msg += this.GenerWhySendToThem(this.HisNode.NodeID, toNode.NodeID);
            ...
            this.addMsg(SendReturnMsgFlag.Rpt, null,"@<a href='/WF/WFRpt.aspx?WorkID=" + this.WorkID + "&FID=" + wk.FID + "&FK_Flow=" + this.HisNode.FK_Flow + "'target='_blank' >工作报告</a>", SendReturnMsgType.Info);
        }
        #endregion
        /// <summary>
        /// 生成标题
        ...
        public void AddIntoWacthDog(GenerWorkerLists gwls)
        {
            ...
            foreach (GenerWorkerList wl in gwls)
            {
                ...
                BP.Sys.SMS.AddMsg(wl.WorkID + "_" + wl.FK_Node + "_" + wl.FK_Emp, wl.FK_Emp, title, title, mytemp);
            }
...
#warning 为了不让其显示在途的工作需要， =3 不是正常的处理模式。
        ...
        private WorkNode GetNextWorkNode()
        {
            // 如果当前工作在没有处理完毕状态,就返回当前的工作.
            if (this.HisWork.NodeState != NodeState.Complete)
                throw new Exception("@此节点的工作任务还没有完成,没有下一步工作."); //"@此节点的工作任务还没有完成,没有下一步工作."

            // 如果他是一个结束节点
            if (this.HisNode.IsEndNode)
                throw new Exception("@此节点是结束节点,没有下一步工作."); // "@此节点是结束节点,没有下一步工作."

    ...
    abstract public class Work : Entity
    {
        ...
        public void DoCopy()
        {
            ...
            if (this.HisRefEnDtl != null)
            {
                ...
                refen.RunSQL("DELETE " + refen.EnMap.PhysicsTable + " WHERE WorkID=" + this.OID);
        ...
        {
            string FK_Taxpayer = "";
            if (this.EnMap.Attrs.Contains("FK_Taxpayer"))
                FK_Taxpayer = this.GetValStringByKey("FK_Taxpayer");

            string paras = "*WorkID" + this.OID + "*UserNo=" + this.Rec + "*NodeState=" + (int)this.NodeState + "*FK_Taxpayer=" + FK_Taxpayer;
        ...
        /// 是不是审核工作
        /// </summary>
        public bool IsGECheckStand
        {
            get
            {
                if (this.ToString() == "BP.WF.GECheckStand" || this.ToString() == "BP.WF.NumCheck")
                    return true;
                else
                    return false;
            }
        ...
        public void DoAutoFull(Attr attr)
        {
            ...
            {
                case BP.Sys.AutoFullWay.Way0:
                    return;
                case BP.Sys.AutoFullWay.Way1_JS:
                    break;
                case BP.Sys.AutoFullWay.Way2_SQL:
                    ...
                    break;
                case BP.Sys.AutoFullWay.Way3_FK:
                    try
                    {
        ...
        public override void Copy(Entity fromEn)
        {
            ...
            foreach (Attr attr in attrs)
            {
                if (attr.Key == WorkAttr.CDT
                    ...
                    || attr.Key == "No"
                    || attr.Key == "Name"
                    || attr.Key == GECheckStandAttr.Note
                    || attr.Key == GECheckStandAttr.Sender)
                    continue;
                if (attr.Key == "RefMsg")

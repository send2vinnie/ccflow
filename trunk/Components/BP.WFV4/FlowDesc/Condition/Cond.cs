...
namespace BP.WF
{
	...
	public class Cond :EntityMyPK
	{
        ...
        public int FK_Attr
        {
            get
            {
                return this.GetValIntByKey(CondAttr.FK_Attr);
            }
            set
            {
                if (value == 0)
                    throw new Exception(" FK_Attr值错误。");
		...
		public object OperatorValue
		{
			...
			set
			{
				this.SetValByKey(CondAttr.OperatorValue,value);
			}
		}
        ...
        public virtual bool IsPassed
        {
            get
            {
                ...
                try
                {
                    ...
                    if (en.IsGECheckStand)
                        en.SetValByKey(GECheckStandAttr.NodeID, this.FK_Node);
                    en.Retrieve();
                }
                catch (Exception ex)
                {
                ...
                try
                {
                    if (en.EnMap.Attrs.Contains(this.AttrKey) == false)
                        throw new Exception("实体：" + nd.EnDesc + " 属性" + this.AttrKey + "已经不存在.");
        ...
        public override Map EnMap
        {
            get
            {
                ...
                map.AddTBInt(CondAttr.FK_Attr, 0, "属性", true, true);
                ...
                map.AddDDLSysEnum(CondAttr.ConnJudgeWay, 0, "条件类型", true, false, CondAttr.ConnJudgeWay, "@0=or@1=and");

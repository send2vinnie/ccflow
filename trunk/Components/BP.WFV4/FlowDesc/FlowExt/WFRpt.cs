    ...
    public class WFRpt : EntityNoName
    {
        ...
        public void DoInitAttrs()
        {
            ...
            if (attrMyNum.OID == 0)
            {
                ...
                attrMyNum.HisEditType = BP.Sys.EditType.UnDel;
                attrMyNum.DefVal = "0";
        ...
        public string DoGenerView()
        {
            ...
            foreach (RptAttr attr in attrs)
            {
                ...
                if (nd.IsCheckNode)
                    sql += ",C" + nd.NodeID + "." + attr.RefField + " AS " + attr.Field;
                else
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                if (nd.IsCheckNode)
                    sql += ",(SELECT * FROM WF_GECheckStand WHERE NODEID=" + mynodeid + " ) C" + mynodeid;
                else
                    sql += "," + dr[0].ToString() + "";
            }
            sql += " WHERE @";
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                if (nd.IsCheckNode)
                    sql += "AND ND" + nodeid + ".OID=C" + mynodeid + ".OID ";
                else

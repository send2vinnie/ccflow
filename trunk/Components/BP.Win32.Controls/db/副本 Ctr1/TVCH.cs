...
//			}
		...
		private void CallOnOfTV_del()
		{
			Log.DebugWriteInfo("Waiting signal ch="+this.No+" pcmn7api.TV_DialRest(this.No) "+pcmn7api.TV_DialRest(this.No)+"Tel="+this.HisCallList.Tel);

			pcmn7api.TV_OffHookCtrl (this.No);
			pcmn7api.TV_StartTimer (this.No, 4);

			Log.DebugWriteInfo("Dialing ch="+this.No+" pcmn7api.TV_DialRest(this.No)= "+pcmn7api.TV_DialRest(this.No)+", Tel="+this.HisCallList.Tel);
			pcmn7api.TV_StartDial (this.No, DataType.StringToByte( this.HisCallList.Tel ));
			while (pcmn7api.TV_DialRest(this.No) > 0)
			{
			}
			Log.DebugWriteInfo("Waiting listener ch="+this.No+" pcmn7api.TV_DialRest(this.No) "+pcmn7api.TV_DialRest(this.No)+"Tel="+this.HisCallList.Tel);
			...
			while(true)
			{
				...
				if (((Sig == 1 || Sig == 2) && SigCount >= 3)) 
				{
					...
					this.HisCallList.Note="忙音"+this.No;
				...
				int i =pcmn7api.TV_ListenerOffHook(this.No);
					 
				if ( i==0 )
				{
					Log.DebugWriteInfo("对方接受者开始接听.TV_ListenerOffHook= "+i +" , ch="+this.No+" Tel="+this.HisCallList.Tel);
		...
		private void CallOnOfTV()
		{
			switch(this.CHState)
			{
				case BP.CTI.CHState.Dialing:  // 正在拨号的状态。
					...
					pcmn7api.TV_StartTimer(this.No,this.RingLong); //设置时间。
				...
				case BP.CTI.CHState.Connect:  // 播完号码后连接状态。
					...
					//					if (((Sig == 1 || Sig == 2) && SigCount >= 3)) 
					//					{
					//						Log.DebugWriteInfo("忙音开始放弃 ch="+this.No+"  Tel="+this.HisCallList.Tel);
					//
					//
					//						this.HisCallList.CallDegree=this.HisCallList.CallDegree+1;
					//						this.HisCallList.CallingStateOfEnum=CallingState.Init;
					...
					int i =pcmn7api.TV_MonitorOffHook(this.No,25) ;

					//int i =pcmn7api.TV_ListenerOffHook(this.No);
					 
					Log.DebugWriteInfo(" TV_MonitorOffHook= "+i+", ch="+this.No+", Tel="+this.HisCallList.Tel);

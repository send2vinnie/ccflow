...
document.write("<iframe name=meizzCalendarIframe scrolling=no frameborder=0 width=151px height=193px></iframe></div>");
function writeIframe()
{
    ...
    "table#tableMain{ width: 149; height: 180;}"+
    ...
    "case 84 : document.forms[0].today.click(); break;} window.event.keyCode = 0; window.event.returnValue= false;}</scr"+"ipt>";
   ...
   "<select name=tmpSecondSelect onblur='parent.hiddenSelect(this)' style='z-index:1; position:absolute;top:171;left:95;display:none'"+
    ...
    "    <td width=63 id=meizzYearHead  title='点击此处选择年份' onclick='parent.funYearSelect(parseInt(this.innerText, 10))'"+
    "        onmouseover='this.bgColor=parent.WebCalendar.darkColor; this.style.color=parent.WebCalendar.lightColor'"+
    "        onmouseout='this.bgColor=parent.WebCalendar.lightColor; this.style.color=parent.WebCalendar.wordColor'></td>"+
    "    <td width=52 id=meizzYearMonth title='点击此处选择月份' onclick='parent.funMonthSelect(parseInt(this.innerText, 10))'"+
    "        onmouseover='this.bgColor=parent.WebCalendar.darkColor; this.style.color=parent.WebCalendar.lightColor'"+
    "        onmouseout='this.bgColor=parent.WebCalendar.lightColor; this.style.color=parent.WebCalendar.wordColor'></td>"+
...
"<td height=20 title='星期日'>日</td><td title='星期一'>一</td><td title='星期二'>二</td><td title='星期三'>三</td><td title='星期四'>四</td><td title='星期五'>五</td><td title='星期六'>六</td></tr></table>"+
    ...
    "    <table id=tableDay height=120 width=147 border=0 cellspacing=1 cellpadding=0>";
         ...
         "</td></tr>" +
         "<tr><td bgcolor='"+ WebCalendar.lightColor +"'><table border=0 cellpadding=0 cellspacing=1 width=147><tr align='center' height=18 bgcolor='"+ WebCalendar.darkColor +"'><td width=49 id=meizzHourHead  title='点击此处选择小时' onclick='parent.funHourSelect(parseInt(this.innerText, 10))'"+
    "        onmouseover='this.bgColor=parent.WebCalendar.darkColor; this.style.color=parent.WebCalendar.lightColor'"+
    "        onmouseout='this.bgColor=parent.WebCalendar.darkColor; this.style.color=parent.WebCalendar.wordColor'></td>"+
       "<td width=49 id=meizzMinuteHead  title='点击此处选择分钟' onclick='parent.funMinuteSelect(parseInt(this.innerText, 10))'"+
    "        onmouseover='this.bgColor=parent.WebCalendar.darkColor; this.style.color=parent.WebCalendar.lightColor'"+
    "        onmouseout='this.bgColor=parent.WebCalendar.darkColor; this.style.color=parent.WebCalendar.wordColor'></td>"+
       "<td width=49 id=meizzSecondHead  title='点击此处选择秒' onclick='parent.funSecondSelect(parseInt(this.innerText, 10))'"+
    "        onmouseover='this.bgColor=parent.WebCalendar.darkColor; this.style.color=parent.WebCalendar.lightColor'"+
...
function WebCalendar() //初始化日历的设置
{
    ...
    this.darkColor  = "#FF6347";    
    this.lightColor = "#FFFFFF";    
    this.btnBgColor = "#FFF5A0";    
    this.wordColor  = "#000040";    
    this.wordDark   = "#DCDCDC";    
    this.dayBgColor = "#FFFACD";    
    this.todayColor = "#FF9933";    
...
{
//this.timeShow= isShowTime;
//WebCalendar = new WebCalendar();

//    WebCalendar_Time(isShowTim);
 ...
 if (arguments.length == 0) WebCalendar.objExport = e;
    ...
    WebCalendar.iframe.tableWeek.style.cursor = "default";
...
function writeCalendar() //对日历显示的数据的处理程序
{
    ...
    if(WebCalendar.timeShow)
    {
      ...
      WebCalendar.iframe.meizzHourHead.innerText    = h +" 时";
      WebCalendar.iframe.meizzMinuteHead.innerText  = mi +" 分";
      //WebCalendar.iframe.meizzSecondHead.innerText  = s +" 秒";
    }
}
       ...
       // returnValue = flag ? a[2] +d[2]+ appendZero(a[1]) +d[2]+ appendZero(a[0]) : a[2] +d[2]+ a[1] +d[2]+ a[0];
        ...
        if(WebCalendar.timeShow)
        {
         ...
         //  var s = WebCalendar.thisSecond;
           ...
           // returnValue += flag ? " "+ appendZero(h) +":"+ appendZero(m) +":"+ appendZero(s) : " "+  h  +":"+ m  ;
...
{
 if (!WebCalendar.timeShow){return;}
   ...
   WebCalendar.thisDay    = new Date().getDate();
    ...
    if(WebCalendar.timeShow)
    {
     ...
     // returnValue += " " + appendZero(WebCalendar.thisHour) + ":" + appendZero(WebCalendar.thisMinute) + ":" + appendZero(WebCalendar.thisSecond);
      returnValue += " " + appendZero(WebCalendar.thisHour) + ":" + appendZero(WebCalendar.thisMinute) ;
    }
    WebCalendar.objExport.value = returnValue;
    hiddenCalendar();
  }

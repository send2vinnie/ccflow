...
namespace Tax666.DataAccess
{
	...
	/// </remarks>
	/// </summary>
	public class ArticleBigTypeDataAccess : IDisposable
	{
		private SqlDataAdapter dsCommand;

		//定义存储过程所用到的参数；
		public const String BigTypeID_Pram			= "@BigTypeID";
		public const String BigTypeName_Pram		= "@BigTypeName";
		public const String BigTypeDesc_Pram		= "@BigTypeDesc";
		...
		public ArticleBigTypeDataAccess()
		{
			/// <summary>
			///  ArticleBigTypeDataAccess 构造函数，创建DataSet Command。
			...
			dsCommand.TableMappings.Add("Table",ArticleBigTypeData.ArticleBigType_Table);
		}
		...
		private ArticleBigTypeData FillArticleBigTypeData(String commandText, String paramName, String paramValue)
		{
			if (dsCommand == null)
			{
				throw new System.ObjectDisposedException(GetType().FullName);
			}
			ArticleBigTypeData data = new ArticleBigTypeData();
		...
		/// <returns>数据实体ArticleBigTypeData</returns>
		private ArticleBigTypeData FillArticleBigTypeData(String commandText)
		{
			if (dsCommand == null)
			{
				throw new System.ObjectDisposedException(GetType().FullName);
		...
		/// <param name="paramValue">sql参数值</param>
		/// <returns>数据实体ArticleBigTypeData</returns>
		private ArticleBigTypeData FillArticleBigTypeData(String commandText,SqlParameter[] prams)
		{
			if (dsCommand == null)
			{
				throw new System.ObjectDisposedException(GetType().FullName);
		...
		private SqlCommand GetInsertCommand()
		{
			...
			{
				insertCommand = new SqlCommand("sp_InsertArticleBigType",new SqlConnection(Tax666Configuration.ConnectionString));
				insertCommand.CommandType = CommandType.StoredProcedure;

				SqlParameterCollection sqlParams = insertCommand.Parameters;

				sqlParams.Add(new SqlParameter(BigTypeName_Pram, SqlDbType.NVarChar,50));
				sqlParams.Add(new SqlParameter(BigTypeDesc_Pram, SqlDbType.NVarChar,200));
                sqlParams.Add(new SqlParameter(Reason_Pram, SqlDbType.Int));

				// 定义数据集中表的参数对应；
				sqlParams[BigTypeName_Pram].SourceColumn	= ArticleBigTypeData.BigTypeName_Field;
				sqlParams[BigTypeDesc_Pram].SourceColumn	= ArticleBigTypeData.BigTypeDesc_Field;
				sqlParams[Reason_Pram].SourceColumn		    = ArticleBigTypeData.Reason_Field;
		...
		public bool InsertArticleBigType(ArticleBigTypeData articleBigTypeData)
		{
			...
			dsCommand.Update(articleBigTypeData, ArticleBigTypeData.ArticleBigType_Table);

			// 检查表错误以查看更新是否成功；
			if (articleBigTypeData.HasErrors)
			{
				articleBigTypeData.Tables[ArticleBigTypeData.ArticleBigType_Table].GetErrors()[0].ClearErrors();
				return false;
			}
			else
			{
		...
		private SqlCommand GetUpdateCommand()
		{
			...
			{
				updateCommand = new SqlCommand("sp_UpdateArticleBigType",new SqlConnection(Tax666Configuration.ConnectionString));
				...
				// 定义数据集中表的参数对应；
				sqlParams[BigTypeID_Pram].SourceColumn		= ArticleBigTypeData.BigTypeID_Field;
				sqlParams[BigTypeName_Pram].SourceColumn	= ArticleBigTypeData.BigTypeName_Field;
				sqlParams[BigTypeDesc_Pram].SourceColumn	= ArticleBigTypeData.BigTypeDesc_Field;
				sqlParams[Reason_Pram].SourceColumn		    = ArticleBigTypeData.Reason_Field;
                sqlParams[Reason_Pram].Direction            = ParameterDirection.Output;
		...
		private SqlCommand GetInfoCommand()
		{
			if (infoCommand == null)
			{
				infoCommand = new SqlCommand("sp_SetArticleBigTypeInfo",new SqlConnection(Tax666Configuration.ConnectionString));
		...
		private SqlCommand GetDeleteCommand()
		{
			...
			{
				deleteCommand = new SqlCommand("sp_DelArticleBigType",new SqlConnection(Tax666Configuration.ConnectionString));
		...
		public ArticleBigTypeData GetArticleBigTypeByListType(int listType,int bigTypeID,int pageSize,int pageIndex,bool doCount)
		{
			...
			prams[3].Value = pageIndex;
			prams[4].Value = doCount;

			return FillArticleBigTypeData("sp_GetArticleBigTypeByListType",prams);
		}

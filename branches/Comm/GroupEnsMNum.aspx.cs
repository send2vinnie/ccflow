    ...
    public partial class GroupEnsNum : BP.Web.WebPage
    {
        ...
        protected void Page_Load(object sender, System.EventArgs e)
        {
            ...
            if (this.IsPostBack == false)
            {
                ...
                if (this.CheckBoxList1.Items.Count == 0)
                    throw new Exception(map.EnDesc + " " + this.ToE("NoFKNoUse", "没有外键条件，不适合做分组查询")); //没有外键条件，不适合做分组查询。
        ...
        public DataTable BingDG()
        {
            ...
            if (orderByReq != null && this.OrderBy != null && (selectSQL.Contains(orderByReq) || groupKey.Contains(orderByReq)))
            {
                ...
                //if (orderByReq != null)
                //{
                //    if (selectSQL.Contains(orderByReq) == true)
                //    {
                //        if (this.OrderWay == "Up")
                //            orderby = " ORDER BY " + orderByReq + " DESC ";
                //        else
            ...
            else  /* 没有合计的情况 */
            {
                ...
                foreach (Attr attr in AttrsOfGroup)
                {
                    if (this.OrderBy == attr.Key)
                    {
                        ...
                        {
                            case "Down":
                                this.UCSys1.AddTDGroupTitle("<a href='GroupEnsMNum.aspx?EnsName=" + this.EnsName + "&OrderBy=" + attr.Key + "&OrderWay=Up' >" + attr.Desc + "<img src='../Images/ArrDown.gif' border=0/></a>");
                                break;
                            case "Up":
                            default:
                                this.UCSys1.AddTDGroupTitle("<a href='GroupEnsMNum.aspx?EnsName=" + this.EnsName + "&OrderBy=" + attr.Key + "&OrderWay=Down' >" + attr.Desc + "<img src='../Images/ArrUp.gif' border=0/></a>");
                                break;
                        }
                    }
                    else
                    {
                        this.UCSys1.AddTDGroupTitle("<a href='GroupEnsMNum.aspx?EnsName=" + this.EnsName + "&OrderBy=" + attr.Key + "&OrderWay=Down' >" + attr.Desc + "</a>");
                    }
                }
                ...
                foreach (Attr attr in AttrsOfNum)
                {
                        ...
                        switch (this.OrderWay)
                        {
                            case "Down":
                                if (this.NumKey == attr.Key)
                                    this.UCSys1.AddTDGroupTitle(lab + "<a href='GroupEnsMNum.aspx?EnsName=" + this.EnsName + "&NumKey=" + attr.Key + "&OrderBy=" + attr.Key + "&OrderWay=Up'><img src='../Images/ArrDown.gif' border=0/></a>");
                                else
                                    this.UCSys1.AddTDGroupTitle("<a href=\"GroupEnsMNum.aspx?EnsName=" + this.EnsName + "&NumKey=" + attr.Key + "\" >" + lab + "</a><a href='GroupEnsMNum.aspx?EnsName=" + this.EnsName + "&NumKey=" + attr.Key + "&OrderBy=" + attr.Key + "&OrderWay=Up'><img src='../Images/ArrDown.gif' border=0/></a>");
                                break;
                            case "Up":
                            default:
                                if (this.NumKey == attr.Key)
                                    this.UCSys1.AddTDGroupTitle(lab + "<a href='GroupEnsMNum.aspx?EnsName=" + this.EnsName + "&OrderBy=" + attr.Key + "&NumKey=" + attr.Key + "&OrderWay=Down'><img src='../Images/ArrUp.gif' border=0/></a>");
                                else
                                    this.UCSys1.AddTDGroupTitle("<a href=\"GroupEnsMNum.aspx?EnsName=" + this.EnsName + "&NumKey=" + attr.Key + "\" >" + lab + "</a><a href='GroupEnsMNum.aspx?EnsName=" + this.EnsName + "&OrderBy=" + attr.Key + "&NumKey=" + attr.Key + "&OrderWay=Down'><img src='../Images/ArrUp.gif' border=0/></a>");
                                break;
                        }
                    }
                    else
                    {
                        if (this.NumKey == attr.Key)
                            this.UCSys1.AddTDGroupTitle(lab + "<a href='GroupEnsMNum.aspx?EnsName=" + this.EnsName + "&NumKey=" + attr.Key + "&OrderBy=" + attr.Key + "' ><img src='../Images/ArrDownUp.gif' border=0/></a>");
                        else
                            this.UCSys1.AddTDGroupTitle("<a href=\"GroupEnsMNum.aspx?EnsName=" + this.EnsName + "&NumKey=" + attr.Key + "\" >" + lab + "</a><a href='GroupEnsMNum.aspx?EnsName=" + this.EnsName + "&NumKey=" + attr.Key + "&OrderBy=" + attr.Key + "' ><img src='../Images/ArrDownUp.gif' border=0/></a>");
            ...
            ur.CfgKey = this.EnsName + "_GroupEnsMNum";
        ...
        {
            //this.ur = new UserRegedit(WebUser.No, this.EnsName + "_GroupEnsMNum");
            //ur.IsPic = this.CB_IsShowPict.Checked;
            //ur.Save();
            this.BingDG();
        }

...
namespace BP.Web.Comm
{
    ...
    /// SimplyNoName 的摘要说明
    /// </summary>
    public partial class PanelEns : WebPage
    {
      ...
      "<input type=button id='ToolBar1$Btn_New' class=Btn name='ToolBar1$Btn_New' onclick=\"javascript:ShowEn('UIEn.aspx?EnsName=" + this.EnsName + "','cd','" + BP.Sys.EnsAppCfgs.GetValInt(this.EnsName, "WinCardH") + "' , '" + BP.Sys.EnsAppCfgs.GetValInt(this.EnsName, "WinCardW") + "');\"  value='" + this.ToE("New", "新建(N)") + "'  />");
        ...
        public Entities SetDGData(int pageIdx)
        {
            ...
            else
            {
                ...
                this.UCSys1.Add("\t\n     location='PanelEns.aspx?EnsName=" + this.EnsName + "&PageIdx=" + PPageIdx + "';");
            }
            ...
            else
            {
                ...
                this.UCSys1.Add("\t\n     location='PanelEns.aspx?EnsName=" + this.EnsName + "&PageIdx=" + ToPageIdx + "';");
            }
                ...
                if (sender.GetType() == typeof(Btn))
                {
                    ...
                    switch (mybtn.ID)
                    {
                        ...
                        case NamesOfBtn.DataGroup:
                            this.Response.Redirect("GroupEnsMNum.aspx?EnsName=" + this.HisEns.ToString(), true);
                            break;
                        default:
                            break;
                    }
        ...
        private void ToolBar1_ButtonClick(object sender, System.EventArgs e)
        {
            try
            {
                ...
                switch (btn.ID)
                {
                    ...
                    //    this.Response.Redirect("PanelEns.aspx?EnsName="+this.EnsName,true);
                    //    return;
